...
FROM (WITH all_data AS 
	...
	JOIN dl_cian.dct_status status
		ON stat.deleted_flag = FALSE
			AND status.bkh = a.status_bkh --LEFT
	JOIN dl_cian.address_actual adr
		ON /*adr.deleted_flag = FALSE AND*/ adr.src_realty_id = a.src_realty_id --AND adr.valid_from = a.valid_from
	...
	FROM 
		...
		LEFT JOIN distinct_price dis
			ON dis.src_cian_id = aa.src_cian_id) a;
                                '''
                                  )
